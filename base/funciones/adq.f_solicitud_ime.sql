...
BEGIN
	...
	***********************************/
	...
	if (p_transaccion='ADQ_SOL_INS') then
        ...
        insert into adq.tsolicitud(
            ...
            v_parametros.proveedor_unico,
            v_parametros.nro_cuotas,
            v_parametros.fecha_ini_cot,
            v_parametros.fecha_ven_cot
		...
		begin
          ...
          -------------------------------------
           ...
           -- si el funcionario que solicita es un gerente .... es el mimso encargado de aprobar
            ...
            proveedor_unico = v_parametros.proveedor_unico,
            fecha_fin = v_parametros.fecha_fin,
            fecha_ini_cot = v_parametros.fecha_ini_cot,
            fecha_ven_cot = v_parametros.fecha_ven_cot,
            nro_cuotas  = v_parametros.nro_cuotas
			where id_solicitud = v_parametros.id_solicitud;
  ...
  /*********************************
 	#TRANSACCION:  'ADQ_INFSEC_GET'
 	#DESCRIPCION:	VERIFICAMOS SI LA SOLICITUD YA CUENTA CON INFORMACION SECUNDARIA
 	#AUTOR:		FEA
 	#FECHA:		07-04-2017 15:12:51
	***********************************/

	elsif(p_transaccion='ADQ_INFSEC_GET')then

		begin
			--Sentencia de la modificacion
			SELECT *
            INTO v_registros
            FROM adq.tinformacion_secundaria tis
			WHERE tis.id_solicitud = v_parametros.id_solicitud;

            --raise exception 'v_registros % ', v_registros is NULL;
			...
			--Definicion de la respuesta
            v_resp = pxp.f_agrega_clave(v_resp,'mensaje','Datos de informacion secundaria.');
            v_resp = pxp.f_agrega_clave(v_resp,'v_id_informacion_sec',v_registros.id_informacion_sec::varchar);
            v_resp = pxp.f_agrega_clave(v_resp,'v_id_solicitud',v_registros.id_solicitud::varchar);
            v_resp = pxp.f_agrega_clave(v_resp,'v_antecedentes',v_registros.antecedentes);
            v_resp = pxp.f_agrega_clave(v_resp,'v_necesidad_contra',v_registros.necesidad_contra);
            v_resp = pxp.f_agrega_clave(v_resp,'v_beneficios_contra',v_registros.beneficios_contra);
            v_resp = pxp.f_agrega_clave(v_resp,'v_resultados',v_registros.resultados);
            v_resp = pxp.f_agrega_clave(v_resp,'v_conclusiones',v_registros.concluciones);
            v_resp = pxp.f_agrega_clave(v_resp,'v_recomendaciones',v_registros.recomendaciones);
            v_resp = pxp.f_agrega_clave(v_resp,'v_validez_oferta',v_registros.validez_oferta);
            v_resp = pxp.f_agrega_clave(v_resp,'v_garantias',v_registros.garantias);
            v_resp = pxp.f_agrega_clave(v_resp,'v_multas',v_registros.multas);
            v_resp = pxp.f_agrega_clave(v_resp,'v_forma_pago',v_registros.forma_pago);
    ...
    /*********************************
 	...
 	#AUTOR:		fea
 	#FECHA:		07-04-2017 18:53:55
	***********************************/

	elsif(p_transaccion='ADQ_CERRAROBS_MOD')then
		...
		begin
            ...
            into
             v_registros
            from wf.tobs obs
            where obs.id_obs=v_parametros.id_obs;
